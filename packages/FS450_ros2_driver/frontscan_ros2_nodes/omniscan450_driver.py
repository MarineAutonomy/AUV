...
"""Driver for Cerulean Omniscan 450 FS (forward scan) over TCP."""
...
SONAR_IP_ADDRESS = '192.168.194.90'
SONAR_PORT = 51200
...
class Omniscan450Driver:
    ...
    def connect(self):
        ...
        try:
            ...
            self._log(
                f'Unable to connect to Omniscan FS at {self.ip_address}:{self.port} — {exc}',
                level='warn')
            return False

        if self.omniscan450.initialize() is False:
            self._log(f'Omniscan FS at {self.ip_address} failed to initialize', level='warn')
            return False

        data = self.omniscan450.readDeviceInformation()
        if data is None:
            self._log(
                f'Omniscan FS at {self.ip_address} did not return device information',
                level='warn')
            return False

        self._log(
            f'Connected to Omniscan FS at {self.ip_address} — device type: {data.device_type}')
        self._set_ping_params(change_speed_of_sound=True, change_os_ping_params=True)
        return True
    ...
    def get_data(self):
        ...
        if not data:
            self._log(
                f'Omniscan FS at {self.ip_address} failed to get message — will reconnect',
                level='warn')
        ...
        if len(scaled_results) == 0:
            self._log(f'Omniscan FS at {self.ip_address}: empty scaled result', level='warn')
    ...
    def set_parameters(self, config):
        change_speed_of_sound = self.speed_of_sound_mm != config['speed_of_sound_mm']
        if change_speed_of_sound:
            self.speed_of_sound_mm = config['speed_of_sound_mm']

        change_os_ping_params = False
        for key in ('start_mm', 'length_mm', 'msec_per_ping', 'pulse_len_percent',
                    'filter_duration_percent', 'gain_index', 'num_results'):
            if getattr(self, key) != config[key]:
                setattr(self, key, config[key])
                change_os_ping_params = True

        if self.is_connected:
            self._set_ping_params(change_speed_of_sound, change_os_ping_params)
        return config

    def _set_ping_params(self, change_speed_of_sound=False, change_os_ping_params=False):
